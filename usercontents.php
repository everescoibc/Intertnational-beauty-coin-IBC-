...
<?php
	...
	$sql00 = mq("SELECT * FROM contents WHERE email = '$useremail' ORDER BY idx DESC"); //전체 게시물 숫자 불러오기

	$total = $sql00->num_rows; //전체 레코드
	if($total < 1) { echo "<h3 style='color: #9F9F9F; text-align: center; padding: 10px;'>아직 작성된 게시물이 없습니다.</h3>"; }

	$num = 5; //한번에 불러올 레코드
	$totalpage = ceil($total/$num); //총 페이지 
	$page = 1; //현재 페이지

	$sql01 = mq("SELECT * FROM contents WHERE email = '$useremail' ORDER BY idx DESC LIMIT $num");  
...
<script>
	...
	$(window).on('scroll', function() {
		...
		if($(window).scrollTop() == readmore) {
			$.ajax( {
				url: "../page/moreusercontents.php",
				data: { pageno: page, useridx: <?php echo $_GET['useridx']; ?> },
				method: "post"
			})
			.done(function(result) {
				$("#more").append(result);
